...
/**
 * Core hover logic for thalo content.
 * Works with either a standalone thalo document or a virtual block document.
 */
...
/**
 * Convert LSP Position to thalo Position (both are 0-based, same structure).
 */
...
export function handleHover(
  ...
  // Handle markdown files with embedded thalo blocks
  if (isMarkdown) {
    const parsed = parseDocument(document.getText(), { fileType: "markdown" });

    // Find which block (if any) contains the cursor position
    const match = findBlockAtPosition(parsed.blocks, lspToThaloPosition(position));
    if (!match) {
      // Position is not inside a thalo block
      return null;
    }

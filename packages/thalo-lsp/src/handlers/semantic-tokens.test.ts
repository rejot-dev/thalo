...
describe("handleSemanticTokens", () => {
  describe("basic token extraction", () => {
    it("should return tokens for a valid thalo document", () => {
...
`);
  ...
  describe("file type detection", () => {
    ...
    it("should handle .md files with thalo blocks", () => {
      const doc = createDocument(
        `# My Document
...
`,
      ...
      // Should extract tokens from the thalo block
      expect(result.data.length).toBeGreaterThan(0);
    });

    it("should handle unknown file extension as thalo", () => {
      const doc = createDocument(
        `2026-01-05T18:00Z create lore "Test" #tag`,
...
`);
  ...
  describe("error handling", () => {
    it("should return empty data for malformed input", () => {
      ...
      const doc = createDocument(`not valid thalo syntax at all
random text here

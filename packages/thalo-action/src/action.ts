...
function generatePRBody(syntheses: SynthesisOutputInfo[], hasFileChanges: boolean): string {
  const entryDetails = syntheses
    .map((s) => {
      const entries = s.entries.length
        ? s.entries
            .map((e) => {
              const link = e.linkId ? ` (^${e.linkId})` : "";
              return `  - ${e.timestamp} ${e.entity}: ${e.title}${link} [${e.file}]`;
            })
            .join("\n")
        : "  - (no entries reported)";

      return `- **${s.title}** (\`^${s.linkId}\`)\n${entries}`;
    })
    .join("\n");
...
export async function runSynthesisAction(inputs: ActionInputs): Promise<void> {
  ...
  if (!hasFileChanges) {
    ...
    // Commit an empty commit so we can open a PR.
    core.info("No file changes from command, creating empty commit");
  }
  ...
  const commitArgs = ["commit", "-m", inputs.commitMessage];
  if (!hasFileChanges) {

 ...
 */
function generatePRBody(syntheses: SynthesisOutputInfo[]): string {
...
async function createOrUpdatePR(
  ...
  const body = generatePRBody(syntheses);
...
export async function runSynthesisAction(inputs: ActionInputs): Promise<void> {
  ...
  if (!statusOutput.stdout.trim()) {
    core.info("No file changes detected after running commands");
    core.setOutput("syntheses-updated", 0);
    return;
  }
  ...
  await createOrUpdatePR(inputs, pending);

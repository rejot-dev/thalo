...
### From a Custom Filesystem

If your files do not live directly on the Node filesystem, use the VFS API instead of the high-level
Node wrappers.
...
`createParser()` expects both `treeSitterWasm` and `languageWasm` to be either raw WASM bytes
(`Uint8Array`) or pre-compiled `WebAssembly.Module` values.

See the dedicated [VFS guide](/docs/vfs) for the full API, browser-safe workspace loading, watch
syncing, and optimistic concurrency control.
...
<Callout>Watching is available in Node.js environments only (uses file system watching).</Callout>

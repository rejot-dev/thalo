...
/**
 * Collect all ptall and markdown files from a directory
 */
...
function actualizeAction(ctx: CommandContext): void {
  ...
  for (const synthesis of syntheses) {
    ...
    console.log(
      `1. Update the content directly below the \`\`\`ptall block in ${relativePath(synthesis.file)}`,
    );
    console.log(`2. Place output BEFORE any subsequent \`\`\`ptall blocks`);
    console.log(
      `3. Append to the ptall block: ${pc.cyan(`actualize-synthesis ^${synthesis.linkId}`)}`,
    );

...
`tree-sitter` package (used by `@rejot-dev/tree-sitter-thalo`) compiles native code but doesn't set
`-std=c++20` in its build config. Without this flag, compilation fails with:
